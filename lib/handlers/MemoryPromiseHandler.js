...
class MemoryPromiseHandler extends Handler {
  ...
  #filterResources(resources, idFilter) {
    if(!idFilter) {
      return [...resources]  // Return a copy to avoid mutating original
    }
  ...
  #sortList(request, list) {
    ...
    list.sort((a, b) => {
      ...
      // Handle null/undefined - sort them to the end based on direction
      if (aVal == null && bVal == null) return 0
      if (aVal == null) return ascending  // ascending: 1 (end), descending: -1 (end when reversed)
      if (bVal == null) return -ascending // ascending: -1 (keep), descending: 1 (keep when reversed)
